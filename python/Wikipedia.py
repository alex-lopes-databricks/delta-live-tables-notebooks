...
from another_file import external_func

# COMMAND ----------

dbutils.fs.mkdirs("/tmp/mydir2")
...
def clickstream_raw():  
    new_dir_name = spark.conf.get("my_dir_name")
    dbutils.fs.mkdirs("/tmp/" + new_dir_name)
    return external_func(spark)
